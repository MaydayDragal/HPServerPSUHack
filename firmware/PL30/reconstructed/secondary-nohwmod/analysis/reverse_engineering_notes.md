# PL30 Rev.10 secondary-side dsPIC33FJ64GS606 reverse-engineering notes
...
## NoHWMod patch versus stock

This package is for the modified NoHWMod secondary image.  Compared with the stock secondary image, the program-memory diff is restricted to 30 words: the ADCBUF2 output-voltage read in the vbus ADC ISR is replaced by a call to a newly inserted Q15 scaling helper, and a float literal in the PWM4/output-command calculation path is raised from `0x46C3EF9E` to `0x46E4EF9E`.  The low-image changes are mirrored in the high application image.

The new helper computes `((uint32_t)ADCBUF2 * 0x6AC2) >> 15`, causing the rest of the firmware to see ~83.4% of the former output-voltage ADC count.  Any unchanged voltage or OVP threshold using `0x0A00`/`0x0A02` therefore corresponds to about 1.199x higher physical output voltage.
