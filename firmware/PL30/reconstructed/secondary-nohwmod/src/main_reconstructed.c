/*
 * PL30 secondary-side PSU firmware reconstruction, NoHWMod/OVP-output-voltage revision.
 * Target: Microchip dsPIC33FJ64GS606, MPLAB X / XC16.
 *
 * This is not Microchip or OEM source.  It is an editable reconstruction from
 * the NoHWMod modified PL30 Rev.10 secondary-side program-memory, SFR,
 * config-bit, and RAM dumps, compared against the stock secondary image.  Addresses from the disassembly are retained in function names and
...
/* NoHWMod patch constants recovered from the binary diff. */
#define SEC_NOHWMOD_ADC_SCALE_Q15          0x6AC2u
#define SEC_NOHWMOD_ADC_SCALE              (27330.0f / 32768.0f)   /* 0.83404541015625 */
#define SEC_NOHWMOD_ADC_APPARENT_GAIN_UP   (32768.0f / 27330.0f)   /* ~1.19898 */
#define SEC_STOCK_OUTPUT_FLOAT_LIMIT_BITS  0x46C3EF9Eu             /* 25079.80859375f */
#define SEC_NOHWMOD_OUTPUT_FLOAT_LIMIT_BITS 0x46E4EF9Eu            /* 29303.80859375f */
...
static uint16_t nohwmod_scale_vout_adc_0x5B00(void);
static uint16_t nohwmod_pwm4_output_limit_0x393A(int16_t signed_error, uint16_t mode_flags);
...
static uint16_t nohwmod_scale_vout_adc_0x5B00(void)
{
    /* Binary patch at low 0x5B00 / high 0xA220:
     *   W1 = ((uint32_t)ADCBUF2 * 0x6AC2) >> 15
     * This makes the firmware see about 83.4045% of the real ADCBUF2 count.
     * Any regulator or OVP code that consumes 0x0A00/0x0A02 therefore trips
     * or regulates at approximately 1.19898x the former physical voltage.
     */
    uint32_t product = (uint32_t)SEC_ADCBUF2 * (uint32_t)SEC_NOHWMOD_ADC_SCALE_Q15;
    return (uint16_t)(product >> 15);
}

static uint16_t nohwmod_pwm4_output_limit_0x393A(int16_t signed_error, uint16_t mode_flags)
{
    /* The stock image used IEEE-754 0x46C3EF9E (25079.8086) in the non-LATG7
     * branch at 0x39B4/0x95B4.  NoHWMod changes only the high word, producing
     * 0x46E4EF9E (29303.8086).  The nearby code converts signed_error to float,
     * applies this constant through the compiler float helpers, converts back
     * to an integer, clamps against 0x5ECC, and ramps PDC4/0x09D8 toward the
     * result.  The exact float helper polarity is preserved in the diff report;
     * this C function exposes the changed editable constant.
     */
    const float stock_limit = 25079.80859375f;
    const float nohwmod_limit = 29303.80859375f;
    const float fallback_limit = 24678.400390625f;  /* branch using 0x46C0CCCD */
    const float lower_clamp = 24268.80078125f;     /* branch using 0x46BD999A / int 0x5ECC */

    float limit = (mode_flags & 0x0002u) ? fallback_limit : nohwmod_limit;
    (void)stock_limit;

    float command = (float)signed_error + limit;
    if (command < lower_clamp) {
        command = lower_clamp;
    }
    if (command > 65535.0f) {
        command = 65535.0f;
...
static void pwm6_control_update_0x34F4(void)
{
    ...
    /* Expose the modified 0x393A/0x953A PWM4/output-command computation.  The
     * stock program writes the result to 0x09EC, then ramps 0x09D8/PDC4.  It is
     * modeled here as a side-effect-free C helper for MPLAB editing.
     */
    uint16_t pwm4_cmd = nohwmod_pwm4_output_limit_0x393A((int16_t)PL30SEC_REG16(0x0DEEu), SEC_POWER_FLAGS);
    PL30SEC_REG16(0x09ECu) = pwm4_cmd;
...
{
    SEC_ADC_VBUS_FAST = nohwmod_scale_vout_adc_0x5B00();
