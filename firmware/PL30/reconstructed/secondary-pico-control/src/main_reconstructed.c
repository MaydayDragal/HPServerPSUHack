/*
 * PL30 secondary-side PSU firmware reconstruction, PICO/RPi-control variant.
 * Target: Microchip dsPIC33FJ64GS606, MPLAB X / XC16.
 *
 * This is not Microchip or OEM source.  It is an editable reconstruction from
 * the modified PICO PL30 Rev.10 secondary-side program-memory, SFR, config-bit, and RAM
 * dumps, compared against the stock Rev.10 secondary image.  Addresses from the disassembly are retained in function names and
...
/* PICO-control patch helpers inserted into free flash/NOPR space.  These are
 * implemented in pico_control_hooks_reconstructed.c and called by patched sites:
 *  - 0x385C/0x945C: OVP ADC scaling helper
 *  - 0x0D38/0x6938: I2C logical register 0x1F write handler
 *  - 0x0D40/0x6940: I2C logical register 0x21 write handler
 *  - 0x2BD6/0x87D6: current ADC substitute helper
 *  - 0x3462/0x9062: external CC-limit clamp/apply helper
 */
...
int main(void)
{
    ...
    for (;;) {
        /* PICO/RPi external control rides on the existing I2C1 register protocol.
         * The modified command table adds writable logical registers 0x1F
         * (CC setpoint) and 0x21 (status: output-enable/CC-enable bits).
         */
